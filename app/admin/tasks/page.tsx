...
// Admin Task Manager — Full visibility across all families, clean layout with task creation
...
export default function AdminTaskManagerPage() {
    ...
    const loadTasks = useCallback(() => {
        ...
        const activeTasks = TaskService.getActiveDesk('admin', user.id, familyIds);
        const snoozed = TaskService.getSnoozedTasks('admin', user.id, familyIds);
        const taskStats = TaskService.getTaskManagerStats('admin', user.id, familyIds);
            ...
            >
                ...
                {viewMode === 'kanban' ? (
                    <KanbanBoard
                        ...
                        onTaskClick={(task) => router.push(`/admin/tasks/${task.id}`)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                        {filteredTasks.map(task => (
                            <TaskManagerCard
                                key={task.id}
                                task={task}
                                onClick={(t) => router.push(`/admin/tasks/${t.id}`)}
                                onSnooze={handleSnooze}
                                onStart={handleStart}
                                onComplete={handleComplete}
                            />
