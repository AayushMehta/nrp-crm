...
// app/back-office/task-manager/page.tsx
// Back Office Task Manager — Clean layout with task creation
...
// ── Families for task creation (mock) ──
...
export default function BackOfficeTaskManagerPage() {
    const { user } = useAuth();
            ...
            >
                ...
                {viewMode === 'kanban' ? (
                    <KanbanBoard
                        ...
                        onTaskClick={(task) => router.push(`/back-office/task-manager/${task.id}`)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                        {filteredTasks.map(task => (
                            <TaskManagerCard
                                key={task.id}
                                task={task}
                                onClick={(t) => router.push(`/back-office/task-manager/${t.id}`)}
                                onSnooze={handleSnooze}
                                onStart={handleStart}
                                onComplete={handleComplete}
                            />
